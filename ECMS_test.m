...
x1=((p_me_0*V_d)/(4*pi));  % First Part to calculate Fuel consumption

y1=J_e*dw_ice; % Second Part to calculate Fuel consumption

costVector=(w_ice/(e*H_l))*(T_ice+x1+y1); % Fuel Consumption Cost Vector for given t_vec
...
% SOC based correction factor
f = 1 - ((soc - 0.5) ./ (0.5* (SOC_max - SOC_min))).^3;
...
if w_ice==0
    ...
    H=P_f+(f * lambda * P_ech); % Hamiltonian calculation
